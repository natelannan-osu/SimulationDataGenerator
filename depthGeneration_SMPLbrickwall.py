...
BVH_TO_SMPL_MAP = {
    # Pelvis and Legs
    "Hips":              "m_avg_Pelvis",
    "LHipJoint":         "m_avg_L_Hip",
    "LeftUpLeg":         None,        # Redundant, mapped to same as above
    "LeftLeg":           "m_avg_L_Knee",
    "LeftFoot":          "m_avg_L_Ankle",
    "LeftToeBase":       "m_avg_L_Foot",
    "RHipJoint":         "m_avg_R_Hip",
    ...
    # Spine and Head
    "LowerBack":         "m_avg_Spine1",
    "Spine":             "m_avg_Spine2",
    "Spine1":            "m_avg_Spine3",
    "Neck":              "m_avg_Neck",
    "Neck1":             "m_avg_Neck",          # Sometimes redundant, keep same
    "Head":              "m_avg_Head",
    ...
    "LeftHandIndex1":    None,                  # Not present in SMPL
    "LThumb":            None,                  # Not present in SMPL
...
# ------------------------------------------------------------
# RETARGETING (with mapping)
# ------------------------------------------------------------
# print("Adding retargeting constraints using BVH_TO_SMPL_MAP...")

# bvh_bones = bvh_armature.pose.bones
# smpl_bones = smpl_armature.pose.bones

# # Create a mapping of bone names for easier lookup
# bvh_bone_names = set(bvh_bones.keys())

# for bvh_name, smpl_name in BVH_TO_SMPL_MAP.items():
#     if smpl_name is None:
#         print(f"Skipping BVH bone '{bvh_name}' (no SMPL target)")
#         continue
#     if bvh_name not in bvh_bone_names:
#         print(f"Skipping missing BVH bone '{bvh_name}'")
#         continue
#     if smpl_name not in smpl_bones:
#         print(f"Skipping missing SMPL bone '{smpl_name}'")
#         continue

#     smpl_bone = smpl_bones[smpl_name]
#     constraint = smpl_bone.constraints.new(type='COPY_ROTATION')
#     constraint.target = bvh_armature
#     constraint.subtarget = bvh_name
#     constraint.target_space = 'POSE'
#     constraint.owner_space = 'POSE'
#     print(f"Added constraint: {smpl_name} ← {bvh_name}")
...
# SAVE BLEND FILE
# ------------------------------------------------------------
bpy.ops.wm.save_as_mainfile(filepath="output/blend/debug.blend")
print(f"[INFO] Scene saved to output/blend/debug.blend")
sys.exit(0)
